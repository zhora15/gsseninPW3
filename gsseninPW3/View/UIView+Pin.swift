...
extension UIView {
    ...
    func pin(to superview: UIView, _ sides: Dictionary<PinSide, Int>){
        for side in sides {
            switch side.key {
            case .top:
    ...
    func pinHeight(to superview: UIView, _ mult: Double = 1) -> NSLayoutConstraint{
        translatesAutoresizingMaskIntoConstraints = false
        let constraint = heightAnchor.constraint(equalTo: superview.heightAnchor, constant: CGFloat(mult))
        constraint.isActive = true
        return constraint
    }
    @discardableResult
